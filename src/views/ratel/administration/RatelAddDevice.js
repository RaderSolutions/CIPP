...
const AddRatelDevice = () => {
  ...
  useEffect(() => {
    ...
    if (deviceType === 'Generic' && callerIdType === 'Custom' && dialplanType === 'Custom') {
      ...
      deviceType === 'Generic' &&
      callerIdType === 'Custom' &&
      dialplanType === 'Default'
    ) {
      setFormFields(
        <div>
          {genericFields}
          {customCallerIDdefaultDialplan}
        </div>,
      )
    } else if (
      deviceType === 'Generic' &&
      callerIdType === 'Default' &&
      dialplanType === 'Custom'
    ) {
      setFormFields(
        <div>
          ...
          <CRow>
            <CCol lg={6} xs={12}>
              ...
              <Select
                ...
                placeholder="Select an option"
                label="Select Device Type:"
                onChange={handleTypeChange(e.target.value, setDeviceType)}
              />
            </CCol>
            <CCol lg={6} xs={12}>
              <RFFCFormSelect
