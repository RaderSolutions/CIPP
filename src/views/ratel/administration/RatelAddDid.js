  ...
  const onSubmit = async (values) => {
    ...
    if (values.DidType === 'Device') {
      ...
      genericPostRequest({ path: '/api/LtRatelDIDS', values: shippedValues })
        ...
        //   DidType: values.DidType,
        // }
        genericPostRequest({ path: `/api/LtScheduleScript?TenantFilter=${tenantDomain}&Parameters=Key=DID|Value=${values.Did},Key=Notes|Value=${values.DialplanName},Key=Dialplan|Value=${values.Dialplan}&RatelScript=true&ScriptId=7532` })
    // genericPostRequest({
